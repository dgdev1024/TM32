...
- The stack pointer is initialized to the RAM/stack memory's ending address
    `0xFFFEFFFF`, growing downward and shrinking upward.
- The stack is considered to be full if `SP` is less than or equal to
    `0xFFFE0003`, and empty if `SP` is at its starting address `0xFFFEFFFF`.
...
The stack pointer begins at address `0xFFFEFFFF`, grows downward with each push
and shrinks upward with each pop.
...
- The stack is considered to be full if `SP` is less than or equal to `0xFFFE0003`.
- The stack is considered to be empty if `SP` is at its starting address `0xFFFEFFFF`.
...
dictated by the client application, via a client-provided function which is to
be called every time a cycle is consumed.
...
- For a full, exhaustive list of all instructions and variants, consult the
    [TM32 CPU Instruction Reference](tm32cpu.instructions.md) document.

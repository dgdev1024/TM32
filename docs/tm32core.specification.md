...
set reference, refer to the [TM32 CPU Instruction Set Reference](tm32cpu.instructions.md).
...
| Start        | End          | Allowed Size    | Description                       |
|--------------|--------------|-----------------|-----------------------------------|
| `0x00000000` | `0x00000FFF` | 4 KB            | Program Metadata                  |
| `0x00001000` | `0x00002FFF` | 12 KB           | Interrupt Service Vectors         |
| `0x00003000` | `0x7FFFFFFF` | Up to 2 GB      | Program Code / Data               |
| `0x80000000` | `0x9FFFFFFF` | Up to 512 MB    | Working Memory (WRAM)             |
| `0xA0000000` | `0xBFFFFFFF` | Up to 512 MB    | External / Save Memory (SRAM)     |
| `0xC0000000` | `0xEFFFFFFF` |                 | Hardware-Reserved Memory          |
| `0xC0008000` | `0xC0009FFF` | 8 KB            | Video RAM (VRAM), Bank 0/1        |
| `0xC000FE00` | `0xC000FE9F` | 160 Bytes       | Object Attribute Memory (OAM)     |
| `0xFFFE0000` | `0xFFFEFFFF` |                 | Stack Memory                      |
| `0xFFFF0000` | `0xFFFFFFFF` |                 | Quick RAM                         |
| `0xFFFFFF00` | `0xFFFFFFFF` | 256 Bytes       | Port Registers                    |
...
#### 4.3.2 Timer Frequencies and Timing
...
#### 4.4.1 Real-Time Clock Overview

**Timekeeping Capabilities:**
- Accurate second, minute, and hour tracking
- Day counter with 16-bit range (0-65535 days)
- Automatic rollover handling for all time units
- Battery-backed operation for persistent timekeeping
- Interrupt generation on time value changes
...
- **Days**: 0-65535 (16-bit counter, approximately 179 years)
...
#### 4.4.2 Real-Time Clock Registers
...
- Rolls over from 59 to 0, incrementing minutes counter
- Read-only, cannot be directly modified by software
- Updated atomically when latch operation is performed
...
**Behavior:**
- Automatically increments when seconds roll over from 59 to 0
- Rolls over from 59 to 0, incrementing hours counter
- Read-only, cannot be directly modified by software
- Updated atomically when latch operation is performed
...
**Behavior:**
- Automatically increments when minutes roll over from 59 to 0
- Rolls over from 23 to 0, incrementing day counter
- Read-only, cannot be directly modified by software
- Updated atomically when latch operation is performed
...
The `RTCD1` and `RTCD0` registers form a 16-bit day counter (0-65535).

**Bit Fields:**
- **RTCD1**: High byte of day counter (bits 15-8)
- **RTCD0**: Low byte of day counter (bits 7-0)
- **Combined**: 16-bit day value (0-65535)

**Behavior:**
- Automatically increments when hours roll over from 23 to 0
- Rolls over from 65535 to 0 (approximately 179-year cycle)
- Read-only, cannot be directly modified by software
- Both bytes updated atomically when latch operation is performed
...
The `RTCL` register is used to latch the current RTC values for atomic reading.

**Behavior:**
- Writing any value to `RTCL` latches current RTC time
- After latching, all RTC read registers contain stable values
...
2. **Automatic Increment**: Time values automatically advance based on real time
3. **Rollover Handling**: Each time unit rolls over to next higher unit when maximum reached
4. **Persistence**: Time continues to advance even when system is powered off
5. **Atomic Access**: Latch mechanism ensures consistent time reading

**Reading RTC Values:**

To read the current time accurately, software must use the latch mechanism:

1. Write any value to `RTCL` register to latch current time
...
#### 4.4.4 RTC Interrupt (Vector 0x06)

The RTC component generates an interrupt whenever any of the read-only time
registers change value. This provides efficient notification of time progression
without requiring constant polling of the RTC registers.

**RTC Interrupt Trigger:**
- **Condition**: Any time register (RTCS, RTCM, RTCH, RTCD1, RTCD0) changes value
- **Frequency**: Once per second (when RTCS increments)
- **Additional Triggers**: When minutes, hours, or days roll over
- **Flag Setting**: Automatically sets bit 6 of `IF0` register
...
calculate_elapsed_time:
    ...
    jpb z, calc_hours       ; Skip if no days
...
days_multiply_loop:
    ...
    jpb nz, days_multiply_loop ; Continue until B = 0
...
calc_hours:
    ...
    jpb z, calc_minutes     ; Skip if no hours
...
hours_multiply_loop:
    ...
    jpb nz, hours_multiply_loop ; Continue until BL = 0
...
calc_minutes:
    ...
    jpb z, calc_seconds     ; Skip if no minutes
...
minutes_multiply_loop:
    ...
    jpb nz, minutes_multiply_loop ; Continue until BL = 0
...
#### 4.6.4 Joypad Interrupt (Vector 0x05)
...
- **Program Metadata (`0x00000000` - `0x00000FFF`)**:
    - Contains a program header with essential information about the program:
        ...
        - `0x00000018`: The program's requested External RAM/Save RAM (SRAM)
            ...
            `0x00001000` (4 KB) and `0x20000000` (512 MB, the maximum
...
#### 5.2.1 Program Loading and Initialization
...
#### 5.3.2 Hardware Component Initialization
...
### 5.4 Programming Patterns and Best Practices

#### 5.4.1 Interrupt Service Routine Guidelines
...
my_function:
    ...
    sub sp, 16                  ; Allocate 16 bytes local space
    
    ; Use [bp-4], [bp-8], etc. for local variables
...
#### 5.4.3 Hardware Component Coordination
...
### 5.6 Advanced Programming Techniques

#### 5.6.1 DMA and HDMA Usage
...
setup_hdma_bg_stream:
    ...
    ; Destination: Background tile data
...
#### 5.6.2 Real-Time Clock Integration
...
safe_memory_access:
    ...
    jpb c, invalid_address     ; Jump if address too low
...
check_stack_overflow:
    ...
    mov a, sp
    cmp a, 0xFFFE0000          ; Check stack lower bound
    jpb c, stack_overflow
    
    cmp a, 0xFFFEFFFF          ; Check stack upper bound
    jpb zc, stack_overflow
...
validate_program_data:
    ...
    ldp a, [player_health]
    cmp a, 0
    jpb c, invalid_health      ; Health cannot be negative
